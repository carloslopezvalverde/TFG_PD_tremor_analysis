function [DTF, P_DTF, band_power, rms_tremor, rms_robust, TPR, temblor_stats] = parametros(signal_X, signal_Y, signal_Z, fs, plot_flag, time_vector)
    % Función para calcular PSD con método de Welch, espectrograma y métricas del temblor
    % Utiliza un solo umbral de 36 dB/Hz para la detección de temblor
    ...
    %% RMS normal del temblor (3-15 Hz)
    rms_tremor = mean([rms(filtered_X), rms(filtered_Y), rms(filtered_Z)]);

    %% 6️⃣ RMS robusto del temblor (3-15 Hz)
    ...
    %% 9️⃣ Umbral para puntos donde hay temblor (un solo umbral: 36 dB)
    umbral_dB = 36;
    ...
    tabla_intervalos = detectar_intervalos(Tx, Fx, Sx_dB, Sx, umbral_dB, 'X', time_vector);
    tabla_intervalos = [tabla_intervalos; detectar_intervalos(Ty, Fy, Sy_dB, Sy, umbral_dB, 'Y', time_vector)];
    tabla_intervalos = [tabla_intervalos; detectar_intervalos(Tz, Fz, Sz_dB, Sz, umbral_dB, 'Z', time_vector)];
    disp('⏱️ Intervalos con potencia > umbral (36 dB):');
    disp(tabla_intervalos);

    % Guardar en base de datos
    assignin('base', 'intervalos_temporales', tabla_intervalos);

    %% Calcular estadísticas de tiempo de temblor
    [t_temblor, t_temblor_porc] = calcular_tiempo_temblor(tabla_intervalos, duracion_total_prueba);
...
%     temblor_stats.tiempo_total = t_temblor;
%     temblor_stats.porcentaje = t_temblor_porc;
    temblor_stats.por_eje = calcular_tiempo_temblor_por_eje(tabla_intervalos, duracion_total_prueba);
    
    % Calcular el tiempo medio y porcentaje medio de los tres ejes
    temblor_stats.tiempo_temblor_medio = mean([temblor_stats.por_eje.X.tiempo_temblor, ...
                                   temblor_stats.por_eje.Y.tiempo_temblor, ...
                                   temblor_stats.por_eje.Z.tiempo_temblor]);
    temblor_stats.porcentaje_temblor_medio = mean([temblor_stats.por_eje.X.porcentaje_temblor, ...
                                       temblor_stats.por_eje.Y.porcentaje_temblor, ...
                                       temblor_stats.por_eje.Z.porcentaje_temblor]);
    ...
    if plot_flag
        ...
        % Señales originales y filtradas + sus PSDs
        subplot(2,3,1);
        plot(signal_X, 'b'); hold on;
        plot(filtered_X, 'r');
        title('Señal X Original y Filtrada'); grid on;
        ylim([-50 50]);  % Normaliza el eje Y a ±50


        subplot(2,3,2);
        plot(f, Pxx_X, 'b'); hold on;
        title('PSD - Eje X'); grid on;
        xlim([0 20]);

        subplot(2,3,3);
        plot(signal_Y, 'b'); hold on;
        plot(filtered_Y, 'r');
        title('Señal Y Original y Filtrada'); grid on;
        ylim([-50 50]);  % Normaliza el eje Y a ±50


        subplot(2,3,4);
        plot(f, Pxx_Y, 'b'); hold on;
        title('PSD - Eje Y'); grid on;
        xlim([0 20]);

        subplot(2,3,5);
        plot(signal_Z, 'b'); hold on;
        plot(filtered_Z, 'r');
        title('Señal Z Original y Filtrada'); grid on;
        ylim([-50 50]);  % Normaliza el eje Y a ±50


        subplot(2,3,6);
        plot(f, Pxx_Z, 'b'); hold on;
        title('PSD - Eje Z'); grid on;
        xlim([0 20]);
        %%
        ...
        % Promedio de los 3 PSD
        plot(f, Pxx_avg, 'k', 'LineWidth', 1.5);
        title('PSD Promediado de los 3 Ejes'); grid on;
...
%%
        % Color para el umbral
        color_umbral = [0.3 0.8 1];  % Azul claro
        ...
        %%
        % Crear una figura con 3 subplots (uno debajo del otro)
        figure('Name', 'Detección de temblor - Ejes X, Y, Z');
        
        % Subplot para el eje X
        subplot(3,1,1);
        hold on;
        % Seleccionar tabla para eje X
        tabla_filtrada = tabla_intervalos(strcmp(tabla_intervalos.Eje, 'X'), :);
        ...
        plot(filtered_X, 'r');
        ylim([-50 50]);
        title(['Señal X con detección de temblor - Umbral ' num2str(umbral_dB) ' dB']);
        ylabel('Amplitud');
        grid on;
        
        % Subplot para el eje Y
        subplot(3,1,2);
        hold on;
        % Seleccionar tabla para eje Y
        tabla_filtrada = tabla_intervalos(strcmp(tabla_intervalos.Eje, 'Y'), :);
        % Pintar zonas de temblor
        for k = 1:height(tabla_filtrada)
            x1 = tabla_filtrada.Inicio_s(k) * fs;
            x2 = tabla_filtrada.Fin_s(k) * fs;
        ...
        plot(filtered_Y, 'r');
        ylim([-50 50]);
        title(['Señal Y con detección de temblor - Umbral ' num2str(umbral_dB) ' dB']);
        ylabel('Amplitud');
        grid on;
        
        % Subplot para el eje Z
        subplot(3,1,3);
        hold on;
        % Seleccionar tabla para eje Z
        tabla_filtrada = tabla_intervalos(strcmp(tabla_intervalos.Eje, 'Z'), :);
        % Pintar zonas de temblor
        for k = 1:height(tabla_filtrada)
            x1 = tabla_filtrada.Inicio_s(k) * fs;
            x2 = tabla_filtrada.Fin_s(k) * fs;
        ...
        % Plot señal original y filtrada
        plot(signal_Z, 'b'); % Cyan para el eje Z
        plot(filtered_Z, 'r');
        ylim([-50 50]);
        title(['Señal Z con detección de temblor - Umbral ' num2str(umbral_dB) ' dB']);
        xlabel('Muestras');
        ylabel('Amplitud');
        grid on;
        ...
        %%
%         %% Gráfico de estadísticas de tiempo de temblor
...
%         tiempos = [temblor_stats.por_eje.X.tiempo_total, ...
%                    temblor_stats.por_eje.Y.tiempo_total, ...
%                    temblor_stats.por_eje.Z.tiempo_total, ...
%                    mean([temblor_stats.por_eje.X.tiempo_total, ...
%                          temblor_stats.por_eje.Y.tiempo_total, ...
%                          temblor_stats.por_eje.Z.tiempo_total])];
%         
%         % Gráfico de barras de tiempo absoluto
%         subplot(2,1,1);
%         bar(tiempos, 'FaceColor', color_umbral);
%         title('Tiempo de temblor por eje');
%         xlabel('Eje');
%         ylabel('Tiempo (s)');
...
%         porcentajes = [temblor_stats.por_eje.X.porcentaje, ...
%                        temblor_stats.por_eje.Y.porcentaje, ...
%                        temblor_stats.por_eje.Z.porcentaje, ...
%                        mean([temblor_stats.por_eje.X.porcentaje, ...
%                              temblor_stats.por_eje.Y.porcentaje, ...
%                              temblor_stats.por_eje.Z.porcentaje])];
%         
%         subplot(2,1,2);
%         bar(porcentajes, 'FaceColor', color_umbral);
%         title('Porcentaje de tiempo con temblor por eje');
%         xlabel('Eje');
%         ylabel('Porcentaje (%)');
%         set(gca, 'XTickLabel', ejes);
%         ylim([0 100]);
...
%     end
    %%
        ...
        ylim([0 20]);                     % Límite de frecuencia
        colormap(jet);                    % Aplicar colormap
        clim([0 60]);                   % Establecer límites de colores
        colorbar;                         % Mostrar el colorbar
        title('Espectrograma con puntos > umbral temblor - Eje X');
        xlabel('Tiempo (s)');
        ylabel('Frecuencia (Hz)');
        hold on;                          % Mantener gráfico para agregar puntos
        [iX, jX] = find(Sx_dB > umbral_dB); % Encontrar índices que superan el umbral
        scatter(Tx(jX), Fx(iX), 10, 'm', 'filled'); % Dibujar puntos morados
        ...
        clim([0 60]);                   % Establecer límites de colores
        colorbar;                         % Mostrar el colorbar
        title('Espectrograma con puntos > umbral temblor - Eje Y');
        xlabel('Tiempo (s)');
        ylabel('Frecuencia (Hz)');
        hold on;                          % Mantener gráfico para agregar puntos
        [iY, jY] = find(Sy_dB > umbral_dB); % Encontrar índices que superan el umbral
        scatter(Ty(jY), Fy(iY), 10, 'm', 'filled'); % Dibujar puntos morados
        ...
        clim([0 60]);                   % Establecer límites de colores
        colorbar;                         % Mostrar el colorbar
        title('Espectrograma con puntos > umbral temblor - Eje Z');
        xlabel('Tiempo (s)');
        ylabel('Frecuencia (Hz)');
        hold on;                          % Mantener gráfico para agregar puntos
        [iZ, jZ] = find(Sz_dB > umbral_dB); % Encontrar índices que superan el umbral
        scatter(Tz(jZ), Fz(iZ), 10, 'm', 'filled'); % Dibujar puntos morados
            end
    ...
    fprintf('RMS del temblor:                              %.4f\n', rms_tremor);
    fprintf('RMS robusto del temblor:                      %.4f\n', rms_robust);
    fprintf('Índice de Potencia del Temblor (TPR):         %.4f %%\n', TPR);
    
    % Mostrar estadísticas de tiempo de temblor
    fprintf('\n⏱️ Estadísticas de tiempo de temblor:\n');
    fprintf('Duración total de la prueba:                  %.2f segundos\n', duracion_total_prueba);
    fprintf('Umbral utilizado:                             %.1f dB\n', umbral_dB);
    fprintf('  Por eje - X: %.2fs (%.2f%%), Y: %.2fs (%.2f%%), Z: %.2fs (%.2f%%)\n', ...
        temblor_stats.por_eje.X.tiempo_temblor, temblor_stats.por_eje.X.porcentaje_temblor, ...
        temblor_stats.por_eje.Y.tiempo_temblor, temblor_stats.por_eje.Y.porcentaje_temblor, ...
        temblor_stats.por_eje.Z.tiempo_temblor, temblor_stats.por_eje.Z.porcentaje_temblor);
end
...
function rms_robust = robust_rms(signal, percentile_threshold)
    % Si no se especifica umbral, usa 95% por defecto
    if nargin < 2
        percentile_threshold = 95;
    end
    
    % Calcula el percentil en el valor absoluto de la señal
    abs_threshold = prctile(abs(signal), percentile_threshold);
    
    % Filtra la señal para mantener solo valores cuya magnitud está por debajo del umbral
    trimmed_signal = signal(abs(signal) <= abs_threshold);
    
    % Calcula el RMS de la señal filtrada
...
function tabla = detectar_intervalos(T, F, S_dB, S, umbral, eje, time_vector)
    ...
    % If no points exceed the threshold, return the empty table
    if ~any(potentes)
        return;
    end
...
%%
% FUNCIONES PARA ESTADÍSTICAS DE TIEMPO DE TEMBLOR
...
function stats_por_eje = calcular_tiempo_temblor_por_eje(tabla_intervalos, duracion_total)
    ...
    % Check if tabla_intervalos is empty or doesn't have the Eje column
    if isempty(tabla_intervalos) || ~ismember('Eje', tabla_intervalos.Properties.VariableNames)
        % If tabla is empty or doesn't have Eje column, initialize all values to zero
        for i = 1:length(ejes)
            eje = ejes{i};
            stats_por_eje.(eje).tiempo_temblor = 0;
            stats_por_eje.(eje).porcentaje_temblor = 0;
            stats_por_eje.(eje).intervalos = table(); % Empty table
        end
        return;
    end
    
    % If tabla_intervalos is valid, continue with normal processing
    for i = 1:length(ejes)
        eje = ejes{i};
        ...
        if isempty(tabla_eje)
            stats_por_eje.(eje).tiempo_temblor = 0;
            stats_por_eje.(eje).porcentaje_temblor = 0;
            stats_por_eje.(eje).intervalos = table(); % Empty table with consistent structure
        else
            % Calcular tiempo total para este eje (considerando posibles solapamientos)
            [tiempo_temblor_eje, porcentaje_temblor_eje] = calcular_tiempo_temblor(tabla_eje, duracion_total);
            
            stats_por_eje.(eje).tiempo_temblor = tiempo_temblor_eje;
            stats_por_eje.(eje).porcentaje_temblor = porcentaje_temblor_eje;
